...
async function main() {
	...
	if (designatedAccount) {
		console.log(
			`ATTEMPTING LIQUIDATION OF DESIGNATED ACCOUNT ${designatedAccount}`,
		)
...
function doConnect() {
	...
	ec.sub(
		...
		(err, patch) => {
			...
			for (let p of patch.result)
				p.path = p.path.split("/").filter((e) => e !== "")
...
async function processAccounts() {
	...
	try {
		...
		reporter.log({
			...
			error: e,
			strategy: bestStrategy && bestStrategy.describe(),
		})
...
async function liquidateDesignatedAccount(violator) {
	...
	console.log(
		`Account ${violator} health = ${ethers.utils.formatEther(
			account.healthScore,
		)}`,
	)
...
async function doLiquidation(act) {
	const { totalLiabilities, totalCollateral } = await getAccountLiquidity(
		act.account,
	)
	act = {
		...act,
	...
	const collaterals = act.markets.filter(
		(m) => m.liquidityStatus.collateralValue !== "0",
	)
	const underlyings = act.markets.filter(
		(m) => m.liquidityStatus.liabilityValue !== "0",
	)
	...
	bestStrategy = opportunities.reduce(
		(accu, o) => {
	...
	if (bestStrategy.best.yield.lt(ethers.utils.parseEther(botConfig.minYield))) {
		...
		reporter.log({
			type: reporter.YIELD_TOO_LOW,
			account: act,
			yield: bestStrategy.best.yield,
			required: botConfig.minYield,
		})
		return false
	}
	...
	reporter.log({
		type: reporter.LIQUIDATION,
		account: act,
		tx,
		strategy: bestStrategy.describe(),
		balanceLeft: botEthBalance,
	})
	return true
}
