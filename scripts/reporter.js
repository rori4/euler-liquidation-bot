...
module.exports = class {
	...
	async report() {
		const countEvent = (events, type) =>
			events.filter((e) => e.type === type).length
		let skipped = 0
		let rep = Object.entries(this.nextReport)
			.map(([account, events]) => {
				// console.log('events: ', events);
				const yieldTooLowCount = countEvent(events, this.YIELD_TOO_LOW)
				const totalCollateral = parseFloat(
					ethers.utils.formatEther(
						events[events.length - 1].account.totalCollateral,
					),
				).toFixed(3)
				...
				const totalLiabilities = parseFloat(
					ethers.utils.formatEther(
						events[events.length - 1].account.totalLiabilities,
					),
				).toFixed(3)
				let latestYield = ""
				if (yieldTooLowCount) {
					latestYield = events
						.filter((e) => e.type === this.YIELD_TOO_LOW)
						.pop().yield
				}
				let msg = ""
				msg = `${account} HS: ${
					events[events.length - 1].account.healthScore / 1000000
				} \n`
				msg += `Total collateral ETH: ${totalCollateral}, Total liabilities ETH: ${totalLiabilities} \n`
				msg += `Yield: ${yieldTooLowCount}${
					yieldTooLowCount &&
					` (${parseFloat(ethers.utils.formatEther(latestYield)).toFixed(6)}) `
				}`
	...
	log(event) {
		...
		if (!this.nextReport[event.account.account])
			this.nextReport[event.account.account] = []
	...
	describeEvent(event) {
		const msg = `${event.time} Account: ${event.account.account} HS: ${
			event.account.healthScore / 1000000
		}`
		switch (event.type) {
			case this.YIELD_TOO_LOW:
				return `${msg} Yield too low (${ethers.utils.formatEther(
					event.yield,
				)} ETH, required ${event.required} ETH)`
			...
			case this.LIQUIDATION:
				return `${msg} LIQUIDATION COMPLETED ${
					event.tx.transactionHash
				} balance left: ${ethers.utils.formatEther(event.balanceLeft)} ${
					event.strategy
				}`
		}
	}
